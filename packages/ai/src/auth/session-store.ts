...
class SessionStore {
  ...
  async setClient(sessionId: string, client: MCPOAuthClient): Promise<void> {
    try {
      // Extract serializable state from the client
      const clientState: SerializableClientState = {
        serverUrl: (client as any).serverUrl,
        callbackUrl: (client as any).callbackUrl,
        oauthProviderState: {
          clientInformation: (client as any).oauthProvider?._clientInformation,
          tokens: (client as any).oauthProvider?._tokens,
          codeVerifier: (client as any).oauthProvider?._codeVerifier,
        },
        isConnected: (client as any).client !== null,
      };
  ...
  async getClient(sessionId: string): Promise<MCPOAuthClient | null> {
    try {
      ...
      // Reconstruct the MCPOAuthClient
      ...
      if (clientState.oauthProviderState) {
        ...
        // Access the private oauthProvider through reflection
