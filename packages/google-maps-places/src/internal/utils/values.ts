...
import { GooglePlacesError } from "../../core/error";
...
export const validatePositiveInteger = (name: string, n: unknown): number => {
  if (typeof n !== "number" || !Number.isInteger(n)) {
    throw new GooglePlacesError(`${name} must be an integer`);
  }
  if (n < 0) {
    throw new GooglePlacesError(`${name} must be a positive integer`);
  }
  return n;
};
