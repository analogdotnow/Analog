...
export async function notificationSourceSeed() {
  const data: (typeof notificationSource.$inferInsert)[] = [
    ...
    },
    {
      ...
      slug: "microsoft",
      description:
        "Microsoft Outlook is an email client that also includes calendar functionality.",
      createdAt: new Date(),
    },
    {
      name: "Local Calendar",
...
export async function notificationSeed(
  ...
  return seedDb.transaction(async (tx) => {
    const randSources = () => rand(sources);
    const sourceEvents: (typeof notificationSourceEvent.$inferInsert)[] =
    ...
    const sourceEventsInserted = await tx
    ...
    if (sourceEventsInserted.length === 0) {
      console.error(
        "No new notification source events were inserted. This might be due to conflicts with existing events.",
      );
      tx.rollback();
      return;
    }

    const randSourceEvent = () =>
      rand(sourceEventsInserted.map((event) => event.id));
    const randNotificationType = () => rand(notificationTypeEnum.enumValues);

    const notifications: (typeof notification.$inferInsert)[] = Array.from(
      { length: amount },
      () => ({
        userId,
