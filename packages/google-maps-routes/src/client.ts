...
import {
  ...
  DistanceMatrixComputeRouteMatrixResponse,
  LocalizedText,
  Money,
  RouteTravelAdvisory,
...
export interface ClientOptions {
  /**
   * Defaults to process.env['GOOGLE_ROUTES_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['GOOGLE_ROUTES_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['GOOGLE_ROUTES_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Google Routes API.
 */
export class GoogleRoutes {
   ...
   * API Client for interfacing with the Google Routes API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['GOOGLE_ROUTES_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['GOOGLE_ROUTES_BASE_URL'] ?? https://routes.googleapis.com] - Override the default base URL for the API.
  ...
  constructor({
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://routes.googleapis.com`,
    };
  ...
  protected stringifyQuery(query: Record<string, unknown>): string {
    return Object.entries(query)
      ...
      .map(([key, value]) => {
        ...
        throw new Errors.GoogleRoutesError(
  ...
  static GoogleRoutes = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static GoogleRoutesError = Errors.GoogleRoutesError;
...
GoogleRoutes.Directions = Directions;
GoogleRoutes.DistanceMatrix = DistanceMatrix;

export declare namespace GoogleRoutes {
