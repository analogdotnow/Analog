...
export function createEventFormMachine({
  ...
  return setup({
    ...
    actions: {
      setFormEvent: assign(({ event }) => ({
        formEvent: (event as QueueEvent).item,
        originalEvent: (event as QueueEvent).item,
        saved: false,
      })),
      queueEvent: assign(({ event }) => ({
        queuedEvent: (event as QueueEvent).item,
      })),
      clearQueue: assign(() => ({ queuedEvent: undefined })),
      updateFormEvent: assign(({ event }) => ({
        formEvent: (event as QueueEvent).item,
      })),
      refreshWithSaved: assign(({ context }) => ({
        formEvent: context.queuedEvent || context.formEvent,
        originalEvent: context.queuedEvent || context.formEvent,
        saved: true,
      })),
      markUnsaved: assign(() => ({ saved: false })),
    },
    guards: {
        ...
        on: {
          QUEUE: [
            ...
            },
            {
              ...
              actions: ["updateFormEvent", "markUnsaved"],
            },
          ],
          SAVE: { target: "saving" },
        },
          ...
          onDone: [
            {
              ...
              target: "ready",
              actions: ["refreshWithSaved", "clearQueue", "markUnsaved"],
            },
            {
              target: "ready",
