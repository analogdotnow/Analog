...
export function Window({ children, className, ...props }: WindowProps) {
  return (
    <motion.div
      className={cn(
        "box-content flex min-h-16 w-full flex-col overflow-hidden rounded-2xl border border-border/20 bg-popover/80 shadow-lg shadow-black/20 backdrop-blur-xl",
        className,
      )}
      {...props}
    >
