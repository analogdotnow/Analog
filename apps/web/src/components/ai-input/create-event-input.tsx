...
export function CreateEventInput() {
  ...
  return (
    ...
    >
      <AnimatePresence>{isEmpty ? <Placeholder /> : null}</AnimatePresence>

      <EditorContent
        editor={editor}
        placeholder="Sales meeting at @22:00"
        className={cn(
          "event-editor-content flex h-10 w-full min-w-0 items-center rounded-sm border bg-sidebar pl-2 text-base",
        )}
      />
    </motion.div>
  );
...
function Placeholder() {
  return (
    <motion.div
      ...
      className="pointer-events-none absolute top-1/2 -translate-y-1/2 p-2 text-muted-foreground"
    >
