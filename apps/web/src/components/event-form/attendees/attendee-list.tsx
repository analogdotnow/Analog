...
function AttendeeActions({ children, className }: AttendeeActionsProps) {
  return (
    <div
      className={cn(
        "hidden items-start gap-1 self-stretch pe-0.5 group-hover/attendee:flex group-has-[[aria-expanded=true]]/attendee:flex group-has-[[data-state=open]]/attendee:flex",
        className,
      )}
    >
      {children}
...
export function AttendeeListItem({
  ...
  return (
    ...
    >
      ...
      <AttendeeStatusIcon
        status={status}
        className="ml-auto pr-1 group-hover/attendee:pr-0 group-has-[[aria-expanded=true]]/attendee:pr-0 group-has-[[data-state=open]]/attendee:pr-0"
      />
      <AttendeeActions>
